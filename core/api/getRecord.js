/* Local modules */
import { findByIDImage } from '../../db/imageQueries.js';
import { generateXml } from '../../utilities/generateXml.js'

/*
Retrieves an image object record and renders either a JSON or XML response.

Parameters:
    req : request
        HTTP request
    res : response
        HTTP response
    next : next  
        Middleware next function
Return:  
    render of 'application/json' or 'application/xml'
*/
export async function getRecord(req, res, next) {
    
    // Get image id from path parameter
    const id = req.params.id;

    // Retrieve image data from database
    const imageData = await findByIDImage(id);

    // Test if query returned a result
    const emptyImageData = (imageData === null
        || Object.entries(imageData).length === 0);

    // Return output based on Accept HTTP header
    res.format({
        // JSON
        'application/json': () => {
            
            if (emptyImageData) {
                res.json({error: 'No image data found.'});
            } else {
                res.json(imageData);
            }
            
        },
        // XML
        'application/xml': () => {

            let imagesXML;
            if (emptyImageData) {
                imagesXML = '<?xml version="1.0">'
                    + '  <errors>'
                    + '    <error>No data available</error>'
                    + '  </errors>'
            } else {
                
                imagesXML = '<?xml version="1.0">'
                + '  <images>'
                + `${generateXml(imageData)}`
                + '  </images>'

            }
            
            res.type('application/xml');
            res.send(imagesXML);
        }
    })

};